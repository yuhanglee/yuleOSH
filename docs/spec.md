...
#### SWR-010.1: CI 配置文件
- CI configuration SHALL be stored in ``.yuleosh/ci-config.yaml``
- The config SHALL support sections for: ci (layers, dependencies), coverage (thresholds, strict), hardware_test (HIL settings)
- Missing config file SHALL fall back to safe defaults
- Invalid YAML SHALL fall back to safe defaults with a warning
- The config SHALL be loaded once and cached per project path
