...
> **最后更新**: 2026-06-15  
> **审计签核**: 小明 🧑‍💼 — Sprint 1~5 质量评估通过，Pipeline/CI/覆盖/规范全部达标
...
### 3.4 ASPICE SWE.1 — 软件需求分析

**已满足：**
- ✅ OpenSpec 规范引擎 (`src/yuleosh/spec/validate.py`) 支持 SHALL/SHOULD/MAY 格式解析
- ✅ spec.md 定义了完整的功能与系统需求（78 条 SHALL, 9 条 SHOULD）
- ✅ RTM 追溯矩阵已建立（`docs/acceptance-matrix-rtm.md`）
- ✅ 需求 ID 命名规范（RS-XXX / SWR-XXX.Y / NFR-XXX）已定义
- ✅ spec-delta 变更追踪机制已就位

**差距：**

| 编号 | ASPICE 要求 | yuleOSH 差距 | 严重度 |
|:----|:------------|:-------------|:------:|
| SWE.1-BP1 | 定义软件需求 | 需求已定义但缺少正式的软件需求文档（SRD） | 🟡 中 |
| SWE.1-BP2 | 结构化需求 | OpenSpec 格式支持结构化但缺少自上而下的需求分解树 | 🟡 中 |
| SWE.1-BP3 | 需求属性 | 缺少正式的需求属性（优先级、复杂度、来源、版本） | 🔴 高 |
| SWE.1-BP4 | 需求双向追溯 | RTM 已定义但双向追溯未完全自动化（正向 OK，反向手动） | 🟡 中 |
| SWE.1-BP5 | 需求评审 | 代码审阅流程存在但缺少形式化的需求评审记录 | 🟡 中 |
| SWE.1-BP6 | 影响分析 | spec-delta 已实现但未与 CI 门禁联动 | 🟡 中 |

**执行计划：**

```
┌──────────────────────────────────────────────────────────────────────┐
│ SWE.1 — 软件需求分析执行计划 (Sprint 1-2 / 2026-Q3)                  │
├──────────────────────────────────────────────────────────────────────┤
│                                                                      │
│ 行动项  | 负责人  | 产出物
│─────────|────────|───────────────────────────────────────────────────│
│ SWE.1-01 | 小马 🐴 | 编写 SWE.1 正式软件需求文档 (docs/swe1-srd.md)     │
│           |         | · 从 spec.md 提取所有软件需求                    │
│           |         | · 增加需求属性（优先级/复杂度/来源/版本）         │
│ SWE.1-02 | 小克 🐰 | 扩展 OpenSpec 引擎支持需求属性                    │
│           |         | · 在 SpecDocument 中添加属性字段                  │
│           |         | · 自动生成需求树 JSON                            │
│ SWE.1-03 | 小马 🐴 | 建立形式化需求评审流程                            │
│           |         | · 定义需求评审 check list                        │
│           |         | · 将评审记录纳入 Evidence Pack                  │
│ SWE.1-04 | 小克 🐰 | RTM 双向追溯自动化                                │
│           |         | · 实现反向追溯（测试→需求）扫描器                 │
│           |         | · 在 CI RTM 门禁中添加反向追溯验证               │
├──────────────────────────────────────────────────────────────────────┤
│ 验收标准：                                                           │
│ · SWE.1-BP1~BP6 全部 AL1 达成                                      │
│ · 需求属性字段覆盖 ≥80%                                             │
│ · 需求评审记录纳入 Evidence Pack                                    │
│ · RTM 双向追溯自动化，覆盖率门禁 ≥80%                              │
└──────────────────────────────────────────────────────────────────────┘
```

### 3.5 ASPICE SWE.2 — 软件架构设计

**已满足：**
- ✅ 多层架构设计：CLI 层 → CI 层 → 后端引擎 → 硬件抽象层
- ✅ `docs/architecture.md` 定义了架构概览
- ✅ 模块划分明确：spec/ci/cross/hardware/evidence/review/pipeline
- ✅ CI pipeline 层次清晰：L0→L1→L2→L2.5→L3
- ✅ 硬件抽象层（FAL / HIL / SIL）接口已设计

**差距：**

| 编号 | ASPICE 要求 | yuleOSH 差距 | 严重度 |
|:----|:------------|:-------------|:------:|
| SWE.2-BP1 | 定义软件架构 | 架构文档 (`docs/architecture.md`) 过于简略，缺少接口定义 | 🔴 高 |
| SWE.2-BP2 | 定义接口 | 模块间接口未形式化定义（缺少接口规格书） | 🔴 高 |
| SWE.2-BP3 | 架构评审 | ❌ 无正式的架构评审流程记录 | 🔴 高 |
| SWE.2-BP4 | 架构评估方法 | ❌ 无 ATAM 或 SAAM 等架构评估记录 | 🟡 中 |
| SWE.2-BP5 | 动态行为 | ❌ 无状态图/顺序图描述模块间交互 | 🟡 中 |

**执行计划：**

```
┌──────────────────────────────────────────────────────────────────────┐
│ SWE.2 — 软件架构设计执行计划 (Sprint 1-2 / 2026-Q3)                  │
├──────────────────────────────────────────────────────────────────────┤
│                                                                      │
│ 行动项  | 负责人  | 产出物
│─────────|────────|───────────────────────────────────────────────────│
│ SWE.2-01 | 老陈 👨‍💼 | 编写正式架构文档 (docs/swe2-architecture.md)     │
│           |         | · 定义模块间接口合约                            │
│           |         | · 绘制模块依赖图 (C4 Model L1/L2)               │
│           |         | · 定义关键接口的数据格式/协议                   │
│ SWE.2-02 | 小克 🐰 | 建立接口契约测试框架                            │
│           |         | · 实现跨模块接口的 contract tests               │
│           |         | · 在 CI 中添加接口契约验证阶段                  │
│ SWE.2-03 | 老陈 👨‍💼 | 架构评审流程 + 记录                            │
│           |         | · 定义架构评审 check list                       │
│           |         | · 首次架构评审记录纳入 Evidence Pack            │
│ SWE.2-04 | 小马 🐴 | 模块接口规格书 (docs/swe2-interface-spec.md)    │
│           |         | · 形式化定义 spec↔ci, ci↔evidence, sil↔hil 接口  │
│           |         | · 定义接口的输入/输出/错误处理协议              │
├──────────────────────────────────────────────────────────────────────┤
│ 验收标准：                                                           │
│ · SWE.2-BP1~BP5 全部 AL1 达成                                      │
│ · 架构文档更新至 C4 Model L2 级别                                  │
│ · 模块间接口合约 ≥80% 已定义                                       │
│ · 架构评审记录 ≥1 次                                                │
└──────────────────────────────────────────────────────────────────────┘
```

### 3.6 ASPICE SWE.3 — 软件详细设计

**已满足：**
- ✅ 模块级详细设计在 `src/` 源码中实现
- ✅ 设计遵循 OOP 原则（抽象基类、接口、依赖注入）
- ✅ `pipeline/run.py`、`ci/run.py` 等核心模块已有内部文档注释
- ✅ 核心设计模式：Factory（FlashTool）、Facade（FlashRunner）、Strategy（HIL/SIL）

**差距：**

| 编号 | ASPICE 要求 | yuleOSH 差距 | 严重度 |
|:----|:------------|:-------------|:------:|
| SWE.3-BP1 | 定义详细设计 | ❌ 缺少正式详细设计文档（只有代码注释） | 🔴 高 |
| SWE.3-BP2 | 设计方法 | OOP 方法已用但无正式设计记录（UML 类图/序列图） | 🔴 高 |
| SWE.3-BP3 | 接口详细设计 | 模块内接口有隐式定义但无正式接口文档 | 🟡 中 |
| SWE.3-BP4 | 行为描述 | 核心流程有伪代码注释但无流程图/状态机 | 🟡 中 |
| SWE.3-BP5 | 数据设计 | ❌ 无数据字典或数据结构规格书 | 🟡 中 |
| SWE.3-BP6 | 设计评审 | ❌ 无正式设计评审记录 | 🔴 高 |

**执行计划：**

```
┌──────────────────────────────────────────────────────────────────────┐
│ SWE.3 — 软件详细设计执行计划 (Sprint 2-3 / 2026-Q3→Q4)               │
├──────────────────────────────────────────────────────────────────────┤
│                                                                      │
│ 行动项  | 负责人  | 产出物
│─────────|────────|───────────────────────────────────────────────────│
│ SWE.3-01 | 小克 🐰 | 编写核心模块详细设计文档 (docs/design/ 目录)    │
│           |         | · pipeline/run.py UML 类图 + 序列图             │
│           |         | · ci/run.py 模块流程图                          │
│           |         | · evidence/pack.py 数据流图                     │
│ SWE.3-02 | 小克 🐰 | 数据结构规格书 (docs/design/data-dictionary.md) │
│           |         | · 核心 dataclass 定义                           │
│           |         | · 关键数据结构的字段/类型/约束                  │
│ SWE.3-03 | 小马 🐴 | 首次正式设计评审 + 记录                          │
│           |         | · 评审 pipeline/run.py 架构                     │
│           |         | · 评审记录归档至 Evidence Pack                 │
│ SWE.3-04 | 小克 🐰 | 自动化设计文档生成                              │
│           |         | · 从源代码注释生成类图/接口图                   │
│           |         | · 纳入 CI 检查确保设计和代码一致               │
├──────────────────────────────────────────────────────────────────────┤
│ 验收标准：                                                           │
│ · SWE.3-BP1~BP6 全部 AL1 达成                                      │
│ · 核心模块 (pipeline, ci, evidence) 均有详细设计文档                │
│ · 数据字典覆盖核心数据结构 ≥80%                                     │
│ · 设计评审记录 ≥1 次                                                │
└──────────────────────────────────────────────────────────────────────┘
```

### 3.7 ISO 26262-8 §8 工具认证差距
