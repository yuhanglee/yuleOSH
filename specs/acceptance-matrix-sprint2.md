...
| # | 发现 | 类型 | 严重度 | 状态 |
|:--|:-----|:----|:------:|:----:|
| R-01 | AC-01 "覆盖率"定义模糊（行/分支/步骤？） | 歧义 | 🟡 中 | ✅ 已确认：分支≥70%（SHALL）+ 行≥80%（SHOULD） |
...
> 小明 🧑‍💼 裁决确认（2026-06-14）：
> - **主指标（SHALL 硬门禁）**: 分支覆盖率 ≥ 70% — `pytest --cov-branch` 验证
> - **辅指标（SHOULD 软门禁）**: 行覆盖率 ≥ 80% — `pytest --cov` 验证
> - **步骤覆盖（SHOULD 参考指标）**: Pipeline 步骤数覆盖率 ≥ 80%（spec 原文定义，与代码覆盖率正交）
...
| AC-01-01a | E2E 测试分支覆盖率 ≥ 70%（主指标） | SHALL | S2-REQ-001.1 | `pytest --cov-branch` | ≥70% | 小克 |
| AC-01-01b | E2E 测试行覆盖率 ≥ 80%（辅指标） | SHOULD | S2-REQ-001.1 | `pytest --cov` | ≥80% | 小克 |
...
| # | 位置 | 意见 | 建议 |
|:--|:-----|:-----|:-----|
| ① | AC-01 | "覆盖率≥80%"定义不明确 | ✅ 已确认：分支覆盖 ≥70%（SHALL 硬门禁）+ 行覆盖 ≥80%（SHOULD 软门禁）。步骤数覆盖率作为正交参考指标保留。 |
